...
#include <linux/workqueue.h> // usada para o agendamento de tarefas
...
void interrupt_routine(struct work_struct *work_arg){
    
    //obtendo a estrutura dev a partir de dwork
    //é feito dessa forma para poder passar argumentos para a função callback
    struct scull_dev *dev = container_of(work_arg,struct scull_dev,dwork.work);
...
ssize_t scull_write(struct file *filp, const char __user *buf,size_t count, loff_t *f_pos){
    ...
    dev->arg = count;
    //adicionando work à work_queue após passar um período de tempo
    queue_delayed_work(dev->my_workqueue,&dev->dwork,time_period);
...
static void __exit scull_exit(void){
    ...
    if(scull_devices){
        for(i=0;i<scull_nr_devs;i++){
            ...
            cancel_delayed_work(&scull_devices[i].dwork);
            flush_workqueue(scull_devices[i].my_workqueue);
            destroy_workqueue(scull_devices[i].my_workqueue);
        }
        kfree(scull_devices);
    }
...
static int __init scull_init(void){
    ...
    for(i=0; i<scull_nr_devs;i++){
        ...
        //criando uma work_queue para cada dispositivo
        scull_devices[i].my_workqueue = create_workqueue("MY_WORK_QUEUE");
        //inicializando a struct work, junto com a rotina de interrupção
        INIT_DELAYED_WORK(&scull_devices[i].dwork,interrupt_routine); 
    }
